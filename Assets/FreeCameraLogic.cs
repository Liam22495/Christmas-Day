...
public class FreeCameraLogic : MonoBehaviour
    ...
    private void Update()
        {
        if (m_targets.Count == 0 || m_currentTarget == null) { return; }

        // Update the angle based on mouse movement
        m_lookAtAroundAngle += Input.GetAxis("Mouse X") * rotationSpeed;

        // Optional: Clamp the angle if needed
        // m_lookAtAroundAngle = Mathf.Clamp(m_lookAtAroundAngle, minAngle, maxAngle);
        }
